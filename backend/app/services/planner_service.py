...
class PlannerService:
    ...
    async def get_plan(
        ...
        available_techniques = ", ".join(
            ...
            if t != CommunicationTechnique.UNKNOWN
        )
        ...
        try:
            data = await self._call_openrouter(
                self.settings.PLANNER_MODEL_NAME, messages
            )
            ...
            if "technique" not in plan_data or plan_data["technique"] not in [
                t.value for t in CommunicationTechnique
            ]:
                raise ValueError(
                    f"Invalid technique returned: {plan_data.get('technique')}"
                )
            ...
            self.log.error(
                "planner_service_json_error", error=str(e), raw_response=content
            )
